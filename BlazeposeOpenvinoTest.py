...
# semaphore_flag = {
#     (3, 4): 'A', (2, 4): 'Gesto 2', (1, 4): 'C', (0, 4): 'D',
#     (4, 7): 'E', (4, 6): 'Gesto 1', (4, 5): 'G', (2, 3): 'H',
#     (0, 3): 'I', (0, 6): 'J', (3, 0): 'K', (3, 7): 'L',
#     (3, 6): 'M', (3, 5): 'N', (2, 1): 'O', (2, 0): 'P',
#     (2, 7): 'Q', (2, 6): 'R', (2, 5): 'S', (1, 0): 'T',
#     (1, 7): 'U', (0, 5): 'V', (7, 6): 'W', (7, 5): 'X',
...
class BlazeposeOpenvino:
    ...
    def load_models(self, pd_xml, pd_device, lm_xml, lm_device):
        ...
        print("{}MKLDNNPlugin version ......... {}.{}".format(" " * 8, versions[pd_device].major,
        ...
        if lm_device != pd_device:
            ...
            print("{}MKLDNNPlugin version ......... {}.{}".format(" " * 8, versions[lm_device].major,
                                                                  versions[lm_device].minor))
    ...
    def lm_render(self, frame, region):
        if region.lm_score > self.lm_score_threshold:
            if self.show_segmentation:
                ...
                src = np.array([[0, 0], [self.segmentation_size, 0], [self.segmentation_size, self.segmentation_size]],
            ...
            if self.use_gesture and self.show_gesture:
                cv2.putText(frame, region.gesture,
                            (region.landmarks_padded[6, 0] - 10, region.landmarks_padded[6, 1] - 50),
    ...
    def run(self):
        ...
        while True:
            if get_new_frame:
                ...
                if self.input_type == "image":
                    ...
                    self.mem_list.append(process.memory_info()[0])
                    cpu_porcentaje = round(process.cpu_percent() / psutil.cpu_count(), 1)
                    self.cpu_list.append(cpu_porcentaje)
            ...
            if not self.force_detection and use_previous_landmarks:
                ...
                mpu.detections_to_rect(self.regions, kp_pair=[0,
                                                              1])  # self.regions.pd_kps are initialized from landmarks on previous frame
                mpu.rect_transformation(self.regions, self.frame_size, self.frame_size)
            else:
                    ...
                    if get_new_frame:
                        if not use_previous_landmarks:
                            ...
                            if r.lm_score > self.lm_score_threshold:
                                ...
                                regions_from_landmarks = [mpu.Region(
                                    pd_kps=r.landmarks_padded[self.nb_lms - 2:self.nb_lms, :2] / self.frame_size)]
            ...
            if self.printmetrics:
                cv2.imshow("Blazepose", met.printMetrics(annotated_frame, frame_actual, num_frames, fps, round(met.Average(self.fps_list), 1), round(max(self.fps_list), 1)))
            ...
            if key == ord('q') or key == 27:
                ...
                self.printmetrics = not self.printmetrics
            elif key == ord('v'):
                self.noview = not self.noview
        ...
        self.fps_list = [i for i in self.fps_list if i > 0.5]
        self.mem_list = [i for i in self.mem_list if i > 0.5]
        self.cpu_list = [i for i in self.cpu_list if i > 0.5]
        resumen = ('\nARGS=' + ' '.join(str(e) for e in sys.argv)
                   + '\nPROGRAM= ' + sys.argv[0]
                   # + '\nFILENAME= ' + sys.argv[0]
                   + '\nFPS_AVG= ' + str(round(met.Average(self.fps_list), 1))
                   + '\nFPS_MAX= ' + str(round(max(self.fps_list), 1))
                   + '\nFPS_MIN= ' + str(round(min(self.fps_list), 1))
                   + '\nMEM_AVG= ' + str(round(met.Average(self.mem_list) / (1024 ** 2), 1)) + 'MB'
                   + '\nMEM_MAX= ' + str(round(max(self.mem_list) / (1024 ** 2), 1)) + 'MB'
                   + '\nMEM_MIN= ' + str(round(min(self.mem_list) / (1024 ** 2), 1)) + 'MB'
                   + '\nCPU_AVG= ' + str(round(met.Average(self.cpu_list), 1)) + '%'
                   + '\nCPU_MAX= ' + str(round(max(self.cpu_list), 1)) + '%'
                   + '\nCPU_MIN= ' + str(round(min(self.cpu_list), 1)) + '%'
                   + '\n')
        print(resumen)
        self.mi_logger.info(resumen)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--noview', action="store_true",
                        help="Se procesa el video sin mostrarlo")
    parser.add_argument('--printmetrics', action="store_true",
                        help="Se agregan las metricas a la imagen de salida")
    ...
    ht = BlazeposeOpenvino(input_src=args.input,
                           ...
                           output=args.output,
                           noview=args.noview,
                           printmetrics=args.printmetrics)
    ht.mi_logger = met.prepareLog('Log_' + sys.argv[0] + '.log', logging.INFO)
